...
function plusSlides(n) {
  ...
  // 슬라이드가 마지막 슬라이드를 넘어가면 처음으로 돌아가도록 설정
  if (currentPosition > 0) {
    currentPosition = -(slides2.length - 1) * slideWidth;
  } else if (currentPosition < -(slides2.length - 1) * slideWidth) {
    currentPosition = 0;
  }
...
//네번째 슬라이드
 let twoSlideIndex4 = 0;
twoShowSlides4();

function twoShowSlides4() {
  let slides2 = document.querySelectorAll('.line2Slide4');
  for (let i = 0; i < slides2.length; i++) {
    slides2[i].style.display = 'none';
  }
  twoSlideIndex4++;
  if (twoSlideIndex4 >= slides2.length) {
    twoSlideIndex4 = 0;
  }
  slides2[twoSlideIndex4].style.display = 'block';
  setTimeout(twoShowSlides4, 2000); // 2초마다 이미지 전환
}

//다섯번째 슬라이드
 let twoSlideIndex5 = 0;
twoShowSlides5();

function twoShowSlides5() {
  let slides2 = document.querySelectorAll('.line2Slide5');
  for (let i = 0; i < slides2.length; i++) {
    slides2[i].style.display = 'none';
  }
  twoSlideIndex5++;
  if (twoSlideIndex5 >= slides2.length) {
    twoSlideIndex5 = 0;
  }
  slides2[twoSlideIndex5].style.display = 'block';
  setTimeout(twoShowSlides5, 2000); // 2초마다 이미지 전환
}
//여섯번째 슬라이드
  let twoSlideIndex6 = 0;
twoShowSlides6();

function twoShowSlides6() {
  let slides2 = document.querySelectorAll('.line2Slide6');
  for (let i = 0; i < slides2.length; i++) {
    slides2[i].style.display = 'none';
  }
  twoSlideIndex6++;
  if (twoSlideIndex6 >= slides2.length) {
    twoSlideIndex6 = 0;
  }
  slides2[twoSlideIndex6].style.display = 'block';
  setTimeout(twoShowSlides6, 2000); // 2초마다 이미지 전환
}
