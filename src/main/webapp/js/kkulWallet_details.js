...
function showSelection() {
   ...
   if (color !== '색상 선택' && eye !== '눈 방향 선택') {
      var selectedOptions = document.getElementById('selected_options');
      var optionKey = color + '_' + eye; // 각 옵션을 식별하는 키

      options[optionKey] = options[optionKey] || { price: 0, quantity: 0 };
      options[optionKey].price = basicPrice; // 기본 가격 설정
      options[optionKey].quantity += 1;


    var newOption = document.createElement('div');
    newOption.innerHTML = '색상: ' + color + ' / 눈 방향: ' + eye + ' <input id="remove" type="button" value=" ✕ " onclick="removeOption(\'' + optionKey + '\')"> <br><span id="amount_' 
    ...
    // 옵션 선택란 초기화
    document.getElementById('color').value = '색상 선택';
    document.getElementById('eye').value = '눈 방향 선택';
...
function changeQuantity(optionKey, value) {
 if (options[optionKey]) {
    ...
    if (options[optionKey].quantity >= 1 && options[optionKey].quantity <= 99) {
      ...
      var totalPrice = basicPrice * options[optionKey].quantity;
...
function updateTotalAmount() {
  ...
  for (var optionKey in options) {
    if (options.hasOwnProperty(optionKey)) {
      totalAmount += basicPrice * options[optionKey].quantity;
    }
  }
